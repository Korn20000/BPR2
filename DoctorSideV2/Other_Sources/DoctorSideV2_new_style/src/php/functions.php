...
function login($email, $password, $mysqli_doctors) {
    ...
    if ($stmt = $mysqli_doctors->prepare("SELECT id, username, password, salt 
				  FROM members 
                                  WHERE email = ? LIMIT 1")) {
        ...
        if ($stmt->num_rows == 1) {
            ...
            if (checkbrute($user_id, $mysqli_doctors) == true) {
                ...
                if ($db_password == $password) {
                    ...
                    $now = time();
                    if (!$mysqli_doctors->query("INSERT INTO login_attempts(user_id, time) 
                                    VALUES ('$user_id', '$now')")) {
...
function checkbrute($user_id, $mysqli_doctors) {
    ...
    if ($stmt = $mysqli_doctors->prepare("SELECT time 
                                  FROM login_attempts 
...
function login_check($mysqli_doctors) {
    ...
    if (isset($_SESSION['user_id'], $_SESSION['username'], $_SESSION['login_string'])) {
        ...
        // Get the user-agent string of the user.
        $user_browser = $_SERVER['HTTP_USER_AGENT'];
        if ($stmt = $mysqli_doctors->prepare("SELECT password 
				      FROM members 
				      WHERE id = ? LIMIT 1")) {
